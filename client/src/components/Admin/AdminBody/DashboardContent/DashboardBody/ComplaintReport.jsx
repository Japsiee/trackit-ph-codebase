...
const ComplaintReport = () => {
    ...
    useEffect(() => {
        browseComplaint()
            .then(data => {
                const filterPending = data.data.filter(complaint => complaint.status === 'pending');
                const filterProcessing = data.data.filter(complaint => complaint.status === 'processing');
                const filterCompleted = data.data.filter(complaint => complaint.status === 'completed');
                const filterCancel = data.data.filter(complaint => complaint.status === 'cancel');
            ...
            const xData = [
                {
                    ...
                    report: <div className='flex items-center'>
                        ...
                        <div className='flex items-center'>
                            <p className="text-neutral-900 text-subbody font-medium ml-6">{report.pending || "0"}</p>
                        </div>
                    </div>,
                    status: <div>
                        <p
                ...
                <div className='grid px-5'>
                    ...
                    >
                        <Column
                            title={
                                <p
                                    ...
                                    style={{ margin: 2 }}> Complaint Report
                                </p>
                            }
                            dataIndex="report"
                            key="report"
                        ...
                        <Column
                            title={
                                <p
                                    className='font-medium text-captiontext-neutral-900'
