...
const IncidentReport = () => {
    ...
    useEffect(() => {
        browseIncident()
            .then(data => {
                const filterPending = data.data.filter(incident => incident.status === 'pending');
                const filterProcessing = data.data.filter(incident => incident.status === 'processing');
                const filterCompleted = data.data.filter(incident => incident.status === 'completed');
                const filterCancel = data.data.filter(incident => incident.status === 'cancel');
                        ...
                        <Column
                            title={
                                <p
                                    ...
                                    style={{ margin: 0 }}> Incident Report
                                </p>
                            }
                            dataIndex="report"
                            key="report"
...
const browseIncident = async () => {
    ...
    const listIncidents = response.json();
